...
void get_defconfig_path(char *out, size_t bufsize, char *argv0)
{
	...
	struct stat info = { 0 };
#ifdef WIN32
	snprintf(out, bufsize, "%s\\yhcminer\\yhcminer-conf.json", getenv("APPDATA"));
#else
	snprintf(out, bufsize, "%s\\.yhcminer\\yhcminer-conf.json", getenv("HOME"));
#endif
	if (dir && stat(out, &info) != 0) {
		snprintf(out, bufsize, "%s%syhcminer-conf.json", dir, sep);
	}
	if (stat(out, &info) != 0) {
		out[0] = '\0';
		return;
